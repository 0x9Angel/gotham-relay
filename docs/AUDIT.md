...
Réponse courte : **tu n'as pas à me croire sur parole.** Le code est public
(AGPL-3.0), tu peux le lire, le recompiler, et vérifier chaque affirmation
ci-dessous toi-même. Ce document pointe la **ligne exacte** qui prouve chaque
garantie.

Tout est dans la crate `crypto-gotham-relay/`. Les renvois sont au format
`fichier:ligne`.

> ℹ️ **Note sur ce dépôt public** : il contient la documentation. Les liens
> `crypto-gotham-relay/src/...` ci-dessous désignent les fichiers de la crate
> du relais dans le dépôt source — demande l'accès en lecture pour suivre les
> renvois ligne à ligne, ou recompile le binaire depuis la source pour
> vérifier l'empreinte (voir « build reproductible » plus bas).
...
➡️ **Conclusion : un opérateur de relais ne peut pas lire les messages, point.**
...
➡️ **Conclusion : aucun relais seul ne peut relier deux interlocuteurs.**
...
➡️ **Conclusion : aucune plainte d'abus type « exit node » n'est possible. Ton
...
## Vérifier toi-même : build reproductible

Tu n'es pas obligé de faire confiance au binaire que je distribue. Recompile
depuis la source et **compare le hash** :

```bash
# 1. Récupère la source publique au tag de la release
git clone <url-du-dépôt> && cd crypto && git checkout vX.Y.Z

# 2. Compile le relais (Rust stable)
cargo build --release -p crypto-gotham-relay --bin gotham-relay

# 3. Compare au binaire publié
sha256sum target/release/gotham-relay
shasum -a 256 -c gotham-relay-<plateforme>.sha256
```

- La même source + le même toolchain produisent le même binaire (build
  déterministe). La CI les compile **sur le runner natif de chaque OS**
  (`.github/workflows/release.yml`, job `relay`), donc rien n'est
  cross-compilé « en douce ».
- Tu peux aussi **lire le code** : la logique d'un relais tient dans
  [process.rs](crypto-gotham-relay/src/process.rs) (~250 lignes), c'est
  volontairement petit et auditable.
